...
// Generic Double-Sided USDZ Builder
export function createUsdz({ widthM, heightM, depthM, uvFront, uvBack, pngBuf }) {
  const W = widthM / 2;
  const H = heightM / 2;
  const D = depthM / 2;
  ...
  // USDZ UV: (0,0) bottom-left
  const frontSt = `[(${p(uvFront.uMin)}, ${p(uvFront.vMin)}), (${p(uvFront.uMax)}, ${p(uvFront.vMin)}), (${p(uvFront.uMax)}, ${p(uvFront.vMax)}), (${p(uvFront.uMin)}, ${p(uvFront.vMax)})]`;
  const backSt = `[(${p(uvBack.uMin)}, ${p(uvBack.vMin)}), (${p(uvBack.uMax)}, ${p(uvBack.vMin)}), (${p(uvBack.uMax)}, ${p(uvBack.vMax)}), (${p(uvBack.uMin)}, ${p(uvBack.vMax)})]`;
...
def Xform "Card"
{
    def Mesh "FrontFace"
    {
        ...
        texCoord2f[] primvars:st = ${frontSt} (
...
`;
  ...
  const files = [
    { name: 'card.usda', data: new TextEncoder().encode(usda) },
    { name: 'card.png', data: pngBuf },
  ];
...
// Generic Double-Sided GLB Builder
export function createGlb({ widthM, heightM, depthM, uvFront, uvBack, pngBuf }) {
  const W = widthM / 2;
  const H = heightM / 2;
  const D = depthM / 2;
  ...
  const UVS = [
    // Front:
    uvFront.uMin, uvFront.vMax,   uvFront.uMax, uvFront.vMax,   uvFront.uMax, uvFront.vMin,   uvFront.uMin, uvFront.vMin,
    // Back:
    uvBack.uMin, uvBack.vMax,     uvBack.uMax, uvBack.vMax,     uvBack.uMax, uvBack.vMin,     uvBack.uMin, uvBack.vMin
  ];
  ...
  const binLen = oImg + pngBuf.length + pad4(pngBuf.length);
  ...
  const json = {
    asset: { version: '2.0', generator: 'inid.me/spatial-card' },
    ...
    materials: [
      {
        ...
        doubleSided: true,
        pbrMetallicRoughness: { baseColorTexture: { index: 0 }, metallicFactor: 0.05, roughnessFactor: 0.35 },
    ...
    bufferViews: [
      ...
      { buffer: 0, byteOffset: oImg, byteLength: pngBuf.length }
    ],
